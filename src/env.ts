 ...
 */
export const ENV_MAX_DOWNLOADS_EACH_RUN: number =
    +process.env.MAX_DOWNLOADS_EACH_RUN || 2;
export const ENV_DIRECTORY = process.env.DIRECTORY ?? '/var/lib/vz/dump';
export const ENV_FILE_NAME =
    process.env.FILE_NAME ??
    'vzdump-qemu-(?<vm>\\d+)-(?<year>\\d+)_(?<month>\\d+)_(?<day>\\d+)-(?<hour>\\d+)_(?<minute>\\d+)_(?<second>\\d+).vma.+';
