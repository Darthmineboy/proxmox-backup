...
if (!existsSync(TEMP_DIR)) {
    ...
    emptyDir(TEMP_DIR).catch((err) =>
        logger.error('Error while emptying temp directory', err),
    );
}
...
export class BackupManager {
    ...
    perform = async (): Promise<void> => {
        ...
        for (const vm of vms) {
            ...
            for (const backup of vmBackups) {
                ...
                if (
                    ENV_MAX_DOWNLOADS_EACH_RUN > 0 &&
                    downloaded === ENV_MAX_DOWNLOADS_EACH_RUN
                ) {
                    logger.info(
                        'Reached the download limit of %s backups per run limit for VM %s',
                        ENV_MAX_DOWNLOADS_EACH_RUN,
                        vm,
                    );
                    break;
                ...
                if (stat.size === sftpInfo.size) {
                    ...
                    await promises.rename(
                        tempFile,
                        path.join(DATA_DIR, backup.name),
                    );
                } else {
                    logger.warn(
                        'Backup size is different, removing backup %s...',
                        backup.name,
                    );
            ...
            for (const backup of vmBackups) {
                if (!backup.remote && !backup.keep) {
                    deleted++;
                    logger.info(
                        'Removing backup %s (not protected by retention)',
                        backup.name,
                    );
            ...
            logger.info(
                'Completed backups for VM %s. Downloaded %s backups, deleted %s backups',
                vm,
                downloaded,
                deleted,
            );
        ...
        logger.info(
            'Completed all backups. Downloaded %s backups, deleted %s backups',
            downloadedTotal,
            deletedTotal,
        );
    };
    ...
    keep(backups: Backup[], keep: number, unit: unitOfTime.StartOf): void {
        ...
        for (let i = 0; i < keep; i++) {
            ...
            if (!backup) {
                logger.debug(
                    'Not enough backups to satisfy full retention period for (period = %s, number = %s)',
                    unit,
                    i + 1,
                );
                break;
            ...
            if (backup.keep) {
                logger.debug(
                    'Backup %s will be kept locally (period = %s, number = %s)',
                    backup.name,
                    unit,
                    i + 1,
                );
                ...
                if (backup.remote) {
                    logger.debug(
                        'Backup %s will be downloaded (period = %s, number = %s)',
                        backup.name,
                        unit,
                        i + 1,
                    );
